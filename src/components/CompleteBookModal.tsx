...
import { X, CheckCircle, BookMarked, Loader2 } from "lucide-react"
...
export default function CompleteBookModal({
  ...
  const [submitting, setSubmitting] = useState<"only" | "excerpt" | null>(null)
  useBodyScrollLock(isOpen)

  useEffect(() => {
    if (!isOpen) setSubmitting(null)
  }, [isOpen])
  ...
  const runComplete = async (mode: "only" | "excerpt") => {
    if (busy) return
    setSubmitting(mode)
    try {
      if (mode === "excerpt") await onCompleteAndOpenExcerpt()
      else await onCompleteOnly()
    } finally {
      setSubmitting(null)
    }
  }
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center overflow-hidden overscroll-none bg-theme-backdrop'>
      {busy ? (
        ...
        >
          <Loader2
            className='h-9 w-9 animate-spin text-accent-theme'
            aria-hidden
          />
          <p className='text-sm font-medium text-theme-primary'>
            완독 처리 중…
          </p>
          <p className='text-xs text-theme-tertiary'>잠시만 기다려 주세요</p>
        </div>
      ) : (
        <div className='modal-dialog-surface mx-4 w-full max-w-md rounded-xl p-6'>
          ...
          <div className='flex flex-col gap-2'>
            <button
              type='button'
              onClick={() => void runComplete("excerpt")}
