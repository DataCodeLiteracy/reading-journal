...
import type { ReadingExamAssessmentJson } from "@/types/readingContent"

interface ReadingExamUploadModalProps {
  isOpen: boolean
  onClose: () => void
  bookTitle: string
  userId: string
...
export default function ReadingExamUploadModal({
  isOpen,
  onClose,
  bookTitle,
  userId,
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!ReadingContentPackService.validateExamJson(parsed)) {
        setError("이해도 점검 JSON 형식이 올바르지 않습니다. assessment_data를 확인해 주세요.")
        return
      }
      setIsUploading(true)
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center overflow-hidden overscroll-none bg-theme-backdrop p-4">
      <div className="modal-dialog-surface w-full max-w-lg max-h-[90vh] overflow-y-auto overflow-x-hidden rounded-xl p-6">
        ...
        <p className="text-sm text-theme-secondary mb-3">
          <strong>{bookTitle}</strong> · 제목 키가 같은 모든 독자에게 공유됩니다.
        </p>
        <input
          ref={fileInputRef}
          type="file"
