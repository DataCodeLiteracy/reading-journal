...
import {
  RECORD_PAGE_SIZE,
  countReviewRecordsPage,
  fetchReviewRecordsPage,
...
export default function ReviewsPage() {
  ...
  const countQuery = useQuery({
    queryKey: queryKeys.record.contentCount(userUid!, "review", scopeKey),
    queryFn: () =>
      countReviewRecordsPage({
        userUid: userUid!,
        showOnlyMine,
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      for (let p = 1; p < currentPage; p++) {
        const batch = await fetchReviewRecordsPage({
          userUid: userUid!,
          showOnlyMine,
          bookId: selectedBookId || undefined,
          searchQuery,
  ...
  return (
    <div className='min-h-screen bg-theme-gradient pb-20'>
      <div className='container mx-auto px-4 py-6'>
          ...
          <div className='space-y-4'>
            ...
            <div>
              ...
              <div className='relative'>
                ...
                <input
                  type='text'
                  placeholder='리뷰 내용, 책 제목, 저자, 사용자 이름으로 검색...'
                  value={searchQuery}
        ...
        {/* 기록 목록 */}
        {isLoading ? (
          <RecordListLoading variant='reviews' />
        ) : records.length === 0 ? (
