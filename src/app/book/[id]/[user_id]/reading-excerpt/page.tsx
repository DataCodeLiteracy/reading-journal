...
function ReadingExcerptHubContent({
  ...
  const [referenceOverallRevealed, setReferenceOverallRevealed] =
    useState(false)
  ...
  useEffect(() => {
    ...
    try {
      if (
        ...
        sessionStorage.getItem(introStorageKey(resolved.id)) === "1"
      ) {
        setStarted(true)
      }
    } catch {
  ...
  const sortedTocEntries = useMemo(
    () => sortBookTocEntries(book?.tocOutline ?? []),
    [book],
  )
  ...
  const displayEntries = useMemo((): Array<{
    path: string
    title: string
    index: number
  }> => {
  ...
  useEffect(() => {
    ...
    const cmap = progress.chapters as Record<
      string,
  ...
  const { doneCount, draftCount } = useMemo(() => {
    ...
    let done = 0,
      draft = 0
    for (let i = 0; i < effectiveCount; i++) {
      const r = (ch[i] ?? (ch as Record<string, unknown>)[String(i)]) as
        | { score?: number; userText?: string }
        | undefined
      if (!r) continue
  ...
  const saveOverall = async () => {
    ...
    setHubError(null)
    try {
      await ReadingExcerptProgressService.upsert(
        userUid,
        resolved.id,
  ...
  if (!book)
    return (
      <div className='p-6 text-theme-secondary'>책을 찾을 수 없습니다.</div>
    )
          ...
          <div className='rounded-lg border border-theme-tertiary bg-theme-secondary p-5 shadow-sm mt-4'>
            ...
            {hasJson ? (
              <ul className='text-sm text-theme-secondary space-y-2 list-disc pl-5 mb-5'>
                <li>
                  챕터별 참고 요약을 보고 나만의 요약을 작성하면 AI가 10점
                  만점과 피드백을 남깁니다.
                </li>
                <li>챕터당 AI 채점은 최대 3회입니다.</li>
                <li>
                  시작 후에는 책 전체 요약·핵심 메시지도 적을 수 있습니다.
                </li>
              </ul>
            ) : (
              <ul className='text-sm text-theme-secondary space-y-2 list-disc pl-5 mb-5'>
                <li>
                  목차가 등록되어 있습니다. 각 챕터에 나만의 요약을 미리 작성해
                  두세요.
                </li>
                <li>
                  참고 자료가 등록되면 참고 요약과 AI 채점을 이용할 수 있습니다.
                </li>
                <li>
                  시작 후에는 책 전체 요약·핵심 메시지도 적을 수 있습니다.
                </li>
            ...
            {pack?.excerptBookMetadata?.overall_summary && (
              <p className='text-xs text-theme-tertiary mb-4'>
                참고 자료에 포함된 전체 요약은 처음에는 가려 두었다가, 탭하면 볼
                수 있습니다.
              </p>
            )}
            <button
            ...
            >
              ...
              <div className='rounded-lg border border-theme-tertiary bg-theme-secondary p-4 shadow-sm'>
                ...
                <p className='mb-2 text-xs text-theme-tertiary'>
                  책 전체를 한 번에 짚는 메모입니다. 챕터별 AI 채점과는 별도로
                  저장됩니다.
                </p>
                <textarea
                  value={overallDraft}
              ...
              <div className='rounded-lg border border-theme-tertiary bg-theme-secondary p-4 shadow-sm'>
                ...
                <p className='mb-3 text-xs text-theme-tertiary'>
                  한 문장 또는 두 문장씩 여러 개를 추가할 수 있습니다. 책
                  상세「핵심」에서 모아서 볼 수 있어요.
                </p>
            ...
            {hasJson && doneCount > 0 && (
              <div className='mb-4 rounded-lg border border-theme-tertiary bg-theme-secondary p-4 text-sm mt-4'>
                <p className='text-theme-primary font-medium'>
                  채점 완료 {doneCount} / {effectiveCount}챕터 · 평균{" "}
                  {avg.toFixed(1)}점 / 10
                </p>
                <p className='text-theme-secondary mt-1'>
                  {labelForAverageScore(avg)}
                </p>
              </div>
            )}
            ...
            <div className='flex items-center justify-between mt-6 mb-1'>
              ...
              {maxDepth > 1 && (
                <div className='flex gap-1'>
                  {Array.from({ length: maxDepth }, (_, i) => i + 1).map(
                    (d) => (
                      <button
                        key={d}
                        type='button'
            ...
            <ul className='space-y-1 mt-1'>
              {displayEntries.map((entry) => {
                ...
                const r = (progress?.chapters?.[entry.index] ??
                  (progress?.chapters as Record<string, unknown> | undefined)?.[
                    String(entry.index)
                  ]) as { score?: number; userText?: string } | undefined
                ...
                return (
                  <li key={entry.path} className='flex items-stretch'>
                    {depth > 0 && (
                      <div style={{ width: `${depth * 2}px`, flexShrink: 0 }} />
                    )}
                    <div className='flex-1 flex rounded-lg border border-theme-tertiary bg-theme-secondary overflow-hidden hover:border-accent-theme transition-colors'>
                      ...
                      >
                        <div className='flex items-start justify-between gap-2'>
                          <span
                            className={`min-w-0 font-medium text-theme-primary ${depth === 0 ? "text-sm" : "text-xs"}`}
                          >
                            <span className='text-theme-tertiary mr-1.5'>
                              {entry.path}
                            </span>
                            {entry.title}
                          </span>
                      ...
                      {canExpand && (
                        <button
                          ...
                          aria-label={
                            expandedPaths.has(entry.path) ? "접기" : "펼치기"
                          }
                        >
